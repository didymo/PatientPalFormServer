...
class CustomView(APIView):
	def get(self, request, format=json):
		...
		#return Response(Analytics().get_stuff())
		route = request.path.strip('/')
		if route == 'get-deployed':
			return JsonResponse(DBCommunicator().return_deployed(), safe=False)
		#TRADE SHOW V
		elif route == 'get-analytics':
			return JsonResponse(Analytics().get_analytics_data(), safe=False)
		##TRADESHOW ^
		else:
	...
	def post(self, request, format=None):
		...
		if request.path.strip('/') == 'form-data':
			...
			##TRADE SHOW V
			return Response(Analytics().store_analytics_data(request))
			##TRADE SHOW ^
		else:
...
class FormDataHandler():
	...
	#OR BETTER YET KEEP THE SECRET KEY WITHIN FIREWALL
